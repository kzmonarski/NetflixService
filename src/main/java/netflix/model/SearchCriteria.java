...
public class SearchCriteria {
	...
	private void validateYear(String title, String year) {
		validatePresenceOfTitle(title, year);
	...
	private void validateYearRange(String year) {
		...
		if (date.isBefore(before) || date.isAfter(LocalDate.now())){
			throw new IllegalArgumentException(DATE_IS_OUT_OF_VALID_RANGE_1900_UP_TILL_NOW);
		}
	}

	private void validatePresenceOfTitle(String title, String year) {
		if (!year.isEmpty() && title.isEmpty()){
			throw new IllegalArgumentException(RELEASE_DATE_CAN_BE_ONLY_PROVIDED_IN_CONJUCTION_WITH_TITLE_NAME);
		}
	}
	...
	private void validateIfEmply(String title, String director, String actor, String year) {
		if (title.isEmpty() && director.isEmpty() && actor.isEmpty() && year.isEmpty()){
			throw new IllegalArgumentException(AT_LEASE_ON_SEARCH_PARAMETER_HAS_TO_BE_PROVIDED);
		}
	}
